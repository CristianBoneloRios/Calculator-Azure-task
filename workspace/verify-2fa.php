...
<body class="auth-page-body">
  ...
  <main class="auth-main">
    <section class="auth-card">
      ...
      <div class="auth-form-panel">
        <h2>Codigo 2FA</h2>
        <p class="auth-muted">Abre tu aplicacion de autenticacion.</p>

        <form id="verify2FAForm" class="auth-form">
          <div style="text-align: center;">
            <label for="twoFactorCode" style="text-align: left; display: block;">Codigo de 6 digitos</label>
            <input type="text" id="twoFactorCode" placeholder="000000" maxlength="6" pattern="[0-9]{6}" inputmode="numeric" style="text-align: center; font-size: 24px; letter-spacing: 8px; font-weight: 700; font-family: 'Courier New', monospace;" required autofocus>
          </div>
          <div class="auth-actions">
            <button type="submit" class="btn btn-primary"><i class="fas fa-check-circle"></i> Verificar codigo</button>
  ...
  <script>
    document.getElementById('verify2FAForm').addEventListener('submit', async event => {
      ...
      if (!/^[0-9]{6}$/.test(code)) {
        alert('El codigo debe ser 6 digitos numericos.');
        return;
      }
      ...
      try {
        ...
        // Redirigir al dashboard del workspace
